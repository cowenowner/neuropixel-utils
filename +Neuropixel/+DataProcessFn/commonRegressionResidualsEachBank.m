function data = commonRegressionResidualsEachBank(imec, data, chIds, timeIdx, extraArg) %#ok<INUSL>
    ...
    % This is laborious, but we do a regression, find the 10 channels say
...
%         [b_lasso] = lasso(X,y,'Lambda',lambda);
mod = fitlm(X,y);
%         [b,bint,r,rint,stats] = regress(y,X);
%         pred = b'*X';
        % apply the model to the data. Get residuals
        % replace data2(ch_ix(iCh),:)
